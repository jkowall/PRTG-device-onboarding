...
async def process_traffic_sensors(
    ...
    for iface in interfaces:
        ...
        if iface['ifadminstatus'] != 1:
            logger.debug("Skipping Interface %s ('%s'): AdminStatus=%s (not Up)",
                         idx, name, iface['ifadminstatus'])
            continue
        if iface['iftype'] not in PHYSICAL_IF_TYPES:
            logger.debug("Skipping Interface %s ('%s'): ifType=%s not in PHYSICAL_IF_TYPES",
                         idx, name, iface['iftype'])
            continue
